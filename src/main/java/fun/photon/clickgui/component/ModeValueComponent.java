...
public class ModeValueComponent extends ValueComponent {
    ...
    public ModeValueComponent(ModeSetting setting, float width) {
    ...
    public void render(double mouseX, double mouseY) {
        ...
        f.drawInRowY(mode.getName(), x + PAD, y, NAME_ROW, fg);

        float top = y + NAME_ROW + 2;
        float maxX = x + PAD + width;
        float px = x + PAD, py = top;

        for (String opt : mode.getOptions()) {
            boolean selected = mode.is(opt);
