...
public class WorldBorderCommand {
                        ...
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                .executes(
                                    p_421393_ -> setSize(
                                        p_421393_.getSource(),
                                        p_421393_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_421393_, "distance"),
                                        0L
                                    )
                                )
                                .then(
                                    Commands.argument("time", TimeArgument.time(0))
                                        .executes(
                                            p_449059_ -> setSize(
                                                p_449059_.getSource(),
                                                p_449059_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_449059_, "distance"),
                                                p_449059_.getSource().getLevel().getWorldBorder().getLerpTime() + IntegerArgumentType.getInteger(p_449059_, "time")
                                            )
                                        )
                                )
                        )
                                ...
                                .then(
                                    Commands.argument("time", TimeArgument.time(0))
                                        .executes(
                                            p_449060_ -> setSize(
                                                p_449060_.getSource(),
                                                DoubleArgumentType.getDouble(p_449060_, "distance"),
                                                IntegerArgumentType.getInteger(p_449060_, "time")
                                            )
                                        )
                                )
                        )
    ...
    private static int setDamageBuffer(CommandSourceStack p_139257_, float p_139258_) throws CommandSyntaxException {
        ...
        if (worldborder.getSafeZone() == p_139258_) {
            throw ERROR_SAME_DAMAGE_BUFFER.create();
        } else {
            worldborder.setSafeZone(p_139258_);
    ...
    private static int setWarningTime(CommandSourceStack p_139260_, int p_139261_) throws CommandSyntaxException {
        ...
        if (worldborder.getWarningTime() == p_139261_) {
            ...
            p_139260_.sendSuccess(() -> Component.translatable("commands.worldborder.warning.time.success", formatTicksToSeconds(p_139261_)), true);
            return p_139261_;
        }
    }
    ...
    private static int setSize(CommandSourceStack p_139253_, double p_139254_, long p_139255_) throws CommandSyntaxException {
        ServerLevel serverlevel = p_139253_.getLevel();
        ...
        if (d0 == p_139254_) {
            ...
            String s = String.format(Locale.ROOT, "%.1f", p_139254_);
            if (p_139255_ > 0L) {
                worldborder.lerpSizeBetween(d0, p_139254_, p_139255_, serverlevel.getGameTime());
                if (p_139254_ > d0) {
                    p_139253_.sendSuccess(() -> Component.translatable("commands.worldborder.set.grow", s, formatTicksToSeconds(p_139255_)), true);
                } else {
                    p_139253_.sendSuccess(() -> Component.translatable("commands.worldborder.set.shrink", s, formatTicksToSeconds(p_139255_)), true);
                }
            } else {
                worldborder.setSize(p_139254_);
                p_139253_.sendSuccess(() -> Component.translatable("commands.worldborder.set.immediate", s), true);
            }
    ...
    private static String formatTicksToSeconds(long p_459424_) {
        return String.format(Locale.ROOT, "%.2f", p_459424_ / 20.0);
    }
}