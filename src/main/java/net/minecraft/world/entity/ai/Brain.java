...
public class Brain<E extends LivingEntity> {
            ...
            (new MapCodec<Brain<E>>() {
                    ...
                    public <T> Stream<T> keys(DynamicOps<T> p_22029_) {
                        return p_21947_.stream()
                            .flatMap(
                                p_22020_ -> p_22020_.getCodec().map(p_449433_ -> BuiltInRegistries.MEMORY_MODULE_TYPE.getKey((MemoryModuleType<?>)p_22020_)).stream()
                            )
                            .map(p_449438_ -> p_22029_.createString(p_449438_.toString()));
                    }
                    ...
                    public <T> DataResult<Brain<E>> decode(DynamicOps<T> p_22022_, MapLike<T> p_22023_) {
                        ...
                        p_22023_.entries()
                            .forEach(
                                p_449436_ -> {
                                    DataResult<MemoryModuleType<?>> dataresult = BuiltInRegistries.MEMORY_MODULE_TYPE.byNameCodec().parse(p_22022_, p_449436_.getFirst());
                                    DataResult<? extends Brain.MemoryValue<?>> dataresult1 = dataresult.flatMap(
                                        p_147350_ -> this.captureRead((MemoryModuleType<T>)p_147350_, p_22022_, (T)p_449436_.getSecond())
                                    );
                                    mutableobject1.setValue(mutableobject1.get().apply2(Builder::add, dataresult1));
                                }
                            );
                        ImmutableList<Brain.MemoryValue<?>> immutablelist = mutableobject1.get()
                            .resultOrPartial(Brain.LOGGER::error)
                            .map(Builder::build)
                            .orElseGet(ImmutableList::of);
                        return DataResult.success(new Brain<>(p_21947_, p_21948_, immutablelist, mutableobject));
                    }
    ...
    public <U> @Nullable Optional<U> getMemoryInternal(MemoryModuleType<U> p_259344_) {
    ...
    public void setSchedule(EnvironmentAttribute<Activity> p_455227_) {
        this.schedule = p_455227_;
    }
    ...
    public void updateActivityFromSchedule(EnvironmentAttributeSystem p_453256_, long p_21863_, Vec3 p_453664_) {
        if (p_21863_ - this.lastScheduleUpdate > 20L) {
            this.lastScheduleUpdate = p_21863_;
            Activity activity = this.schedule != null ? p_453256_.getValue(this.schedule, p_453664_) : Activity.IDLE;
